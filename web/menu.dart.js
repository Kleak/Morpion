...
$$.Closure$playClicked = {"": "Closure;call$1,$name"};
...
["KeyGenerator", "KeyGenerator.dart", , {
KeyGenerator: {"": "Object;keyValues,keyValuesLen,len",
  generate$0: function() {
    var t1, t2, t3, res, i, max, randNb;
    C.C__Random.nextInt$1(this.keyValuesLen);
    for (t1 = this.len, t2 = this.keyValues, t3 = t2.length, res = "", i = 0; i < t1; ++i) {
      max = this.keyValuesLen;
      if (max < 0)
        $.throwExpression(new $.ArgumentError("negative max: " + max));
      if (max > 4294967295)
        max = 4294967295;
      randNb = Math.random() * max >>> 0;
      if (randNb < 0 || randNb >= t3)
        throw $.ioore(randNb);
      res = C.JSString_methods.$add(res, t2[randNb]);
    }
    return res;
  }
}}],
...
_EventLoop: {"": "Object;events,activeTimerCount",
  ...
  checkOpenReceivePortsFromCommandLine$0: function() {
    if ($globalState.rootContext != null && $globalState.isolates.containsKey$1($globalState.rootContext.id) && $globalState.fromCommandLine === true && $.$eq($globalState.rootContext.ports._length, 0))
...
HashMapKeyIterator: {"": "Object;_map,_liblib2$_keys,_offset,_current",
  get$current: function() {
...
HashMap: {"": "Object;_length,_strings,_nums,_rest,_liblib2$_keys",
  get$length: function(_) {
    return this._length;
  },
  get$isEmpty: function(_) {
...
LinkedHashMap: {"": "Object;_length,_strings,_nums,_rest,_first,_last,_modifications",
  ...
  _unlinkCell$1: function(cell) {
    ...
    if (next == null)
      ...
      next._liblib2$_previous = previous;
...
_ChildNodeListLazy: {"": "ListBase;_this",
  ...
  addAll$1: function(_, iterable) {
    ...
    return;
    for (t1 = C.NodeList_methods.get$iterator(iterable._this.childNodes), t2 = this._this; t1.moveNext$0();)
...
_ElementListEventStreamImpl: {"": "Stream;_pool,_liblib1$_stream",
  listen$4$cancelOnError$onDone$onError: function(onData, cancelOnError, onDone, onError) {
    return this._liblib1$_stream.listen$4$cancelOnError$onDone$onError(onData, cancelOnError, onDone, onError);
  },
  listen$3$onDone$onError: function(onData, onDone, onError) {
    return this.listen$4$cancelOnError$onDone$onError(onData, null, onDone, onError);
  }
...
_StreamPool: {"": "Object;_liblib1$_controller,_subscriptions"},
...
_Html5NodeValidator: {"": "Object;uriPolicy<",
  ...
  _Html5NodeValidator$1$uriPolicy: function(uriPolicy) {
    var t1, attr, t2;
    if ($.$eq($.get$_Html5NodeValidator__attributeValidators()._length, 0)) {
      for (t1 = new $.ListIterator(C.List_1GN, 261, 0, null); t1.moveNext$0();) {
        attr = t1._liblib0$_current;
...
_TemplatingNodeValidator: {"": "_SimpleNodeValidator;_templateAttrs,allowedElements,allowedAttributes,allowedUriAttributes,uriPolicy",
  allowsAttribute$3: function(element, attributeName, value) {
    ...
    if (attributeName === "template" && value === "")
      return true;
    if ($.get$attributes$x(element)._element.getAttribute("template") === "") {
  ...
  sanitizeNode$1: function(node) {
    ...
    switch (t1.get$nodeType(node)) {
      case 1:
        ...
        keys = $.List_List$from(attrs.get$keys(), true);
        for (i = attrs.get$keys().length - 1; i >= 0; --i) {
          if (i >= keys.length)
            throw $.ioore(i);
          $name = keys[i];
          if (this.validator.allowsAttribute$3(node, $.toLowerCase$0$s($name), t2.getAttribute($name)) !== true) {
            window;
            $.get$Console__safeConsole();
            t3 = "Removing disallowed attribute <" + t1.get$tagName(node) + " " + $.S($name) + "=\"" + t2.getAttribute($name) + "\">";
            if (typeof console != "undefined")
              console.warn(t3);
...
DocumentFragment: {"": "Node;",
  ...
  setInnerHtml$3$treeSanitizer$validator: function(receiver, html, treeSanitizer, validator) {
    ...
    receiver.appendChild($.createFragment$3$treeSanitizer$validator$x(document.body, html, treeSanitizer, validator));
  },
...
Element: {"": "Node;style=,tagName=",
  ...
  createFragment$3$treeSanitizer$validator: function(receiver, html, treeSanitizer, validator) {
    var t1, t2, contextElement, range, fragment;
    ...
    if (t1) {
      range = t2.createRange();
      range.selectNodeContents(contextElement);
      fragment = range.createContextualFragment(html);
    } else {
      contextElement.innerHTML = html;
      fragment = t2.createDocumentFragment();
...
InputElement: {"": "HtmlElement;name=,value=", $isElement: true, $asElement: null},
...
playClicked: function(e) {
  var kg, key;
  kg = new $.KeyGenerator("0123456789abcdefghijklmnopqrstuvwxyz", null, 24);
  kg.keyValuesLen = kg.keyValues.length;
  key = kg.generate$0();
  $.assign$1$x(C.Window_methods.get$location(window), "/home/duo/" + key);
},

main: function() {
  var playButton, t1, playLabel;
  $.set$margin$x(document.body.style, "0");
  $.set$padding$x(document.body.style, "0");
  $.set$backgroundColor$x(document.body.style, "#F8F8F8");
  $.set$textAlign$x(document.querySelector("#mainTitle").style, "center");
  playButton = document.createElement("div");
  $.set$width$x(playButton.style, "400px");
  $.set$height$x(playButton.style, "200px");
  $.set$margin$x(playButton.style, "auto");
  $.set$position$x(playButton.style, "relative");
  $.set$backgroundColor$x(playButton.style, "white");
  $.set$borderRadius$x(playButton.style, "30px 30px 30px 30px");
  $.set$cursor$x(playButton.style, "pointer");
  C.EventStreamProvider_click.forElement$2$useCapture;
  t1 = new $._ElementEventStreamImpl(playButton, C.EventStreamProvider_click._eventType, false);
  new $._EventStreamSubscription(0, t1._target, t1._eventType, $.playClicked$closure, t1._useCapture)._tryResume$0();
  document.body.appendChild(playButton);
  playLabel = document.createElement("h1");
  t1 = $.getInterceptor$x(playLabel);
  $.set$position$x(t1.get$style(playLabel), "relative");
  $.set$fontSize$x(t1.get$style(playLabel), "50px");
  $.set$top$x(t1.get$style(playLabel), "70px");
  $.set$textAlign$x(t1.get$style(playLabel), "center");
  t1.set$innerHtml(playLabel, "PLAY");
...
$.playClicked$closure = new $.Closure$playClicked($.playClicked, "playClicked$closure");
...
// Native classes
$.defineNativeMethods("DOMError|DOMImplementation|FileError|MediaError|MediaKeyError|Navigator|NavigatorUserMediaError|PositionError|SQLError|SVGAnimatedNumberList", $.Interceptor);
...
//# sourceMappingURL=menu.dart.js.map
//@ sourceMappingURL=menu.dart.js.map
